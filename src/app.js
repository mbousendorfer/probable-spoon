...
function renderBriefEntryEditor(composer) {
  return (
    ...
    '<input class="brief-inline-input" type="text" placeholder="Label (optional)" value="' +
    escapeHtml(composer.label) +
    '" data-brief-field="label" />' +
    '<input class="brief-inline-input" type="text" placeholder="Value" value="' +
    escapeHtml(composer.value) +
    '" data-brief-field="value" />' +
    "</div></div>"
  );
}
...
function renderBriefSection(session, ui, section) {
  ...
  return (
    ...
    (section.entries.length
      ? iconButton({
          label: "Refine " + section.title + " with AI",
          icon: icons.sparkles,
          attrs:
            'data-refine-brief="' +
            section.id +
            '"' +
            (isRefining ? ' disabled aria-disabled="true"' : ""),
          stroked: true,
        })
      : "") +
    '</div><div class="step-card strategy-section__card">' +
    (section.entries.length
        ...
        section.entries
          .map((entry) => {
            ...
            return (
              ...
              '" data-section-id="' +
              section.id +
              '"><span class="strategy-entry__label">' +
              escapeHtml(entry.label || "Entry") +
              '</span><span class="strategy-entry__value">' +
              formatText(entry.value) +
              '</span></button><div class="strategy-entry__actions">' +
              iconButton({
                label: "Delete entry",
                icon: icons.trash,
                attrs:
      ...
      : '<div class="inline-empty-state strategy-section__empty"><div class="icon">' +
        briefSectionIcon(section.icon) +
        '</div><p>No entries yet. Add the first detail for this section.</p></div>') +
    (isComposerInSection && composer.mode === "add-entry" ? renderBriefEntryEditor(composer) : "") +
...
function renderStrategyBriefView(session, ui) {
  const { sectionCount, entryCount } = strategyBriefMeta(session);
  const brief = session.strategyBrief;
  ...
  return (
    ...
    " sections</span> · <span>" +
    entryCount +
    ' entries</span></div></div></div><div class="strategy-brief-layout">' +
    (brief.sections.length
      ...
      : '<div class="empty-state"><div class="icon">' +
        icons.fileText +
        '</div><h3 style="margin-top: 18px; color: var(--ref-color-grey-150)">No strategy sections yet</h3><p>Start with a section for goals, audience, or brand voice, then add entries inline as the brief takes shape.</p></div>') +
    (isAddingSection
    ...
    actionButton({
      ...
      disabled: ui.pendingBriefRefine || entryCount === 0,
    }) +
    ...
    (section.entries?.length
      ? '<div class="strategy-entry-list">' +
        section.entries
          .map(
            (entry) =>
              '<div class="strategy-entry"><div class="strategy-entry__trigger" role="group"><span class="strategy-entry__label">' +
              escapeHtml(entry.label || "Entry") +
              '</span><span class="strategy-entry__value">' +
              formatText(entry.value) +
              "</span></div></div>",
...
function messageMarkup(message) {
  if (message.role === "system") {
    return (
      '<div class="assistant-notice"><div class="assistant-turn__meta"><span class="assistant-turn__role">' +
      icons.info +
      "<span>" +
      escapeHtml(message.meta) +
      '</span></span></div><div class="assistant-turn__content">' +
...
function buildPostsRailItems(session) {
  ...
  return {
    views: [
      { id: "all-posts", label: "All posts", count: posts.length, kind: "all" },
      { id: "needs-fixes", label: "Needs fixes", count: posts.filter((post) => validatePostDraft(post).length > 0).length, kind: "status", value: "needs_fixes" },
      { id: "ready-posts", label: "Ready", count: posts.filter((post) => post.status === "ready" && post.workflowState === "draft" && validatePostDraft(post).length === 0).length, kind: "status", value: "ready" },
      { id: "prepared-posts", label: "Prepared", count: posts.filter((post) => post.workflowState === "prepared").length, kind: "status", value: "prepared" },
      { id: "scheduled-posts", label: "Scheduled", count: posts.filter((post) => post.workflowState === "scheduled").length, kind: "status", value: "scheduled" },
      { id: "best-drafts", label: "Best drafts", count: posts.filter((post) => post.aiSuggested).length, kind: "best" },
    ],
    networks: [
...
function postsWorkspaceViewLabel(ui) {
  if (ui.postsActiveRailView === "best-drafts") {
    return ui.postsShowSelectedOnly ? "Best drafts · Selected only" : "Best drafts";
  }
