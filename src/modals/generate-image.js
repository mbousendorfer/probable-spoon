 ...
 */
async function generateImage(prompt, seed) { // eslint-disable-line no-unused-vars
...
function getModal()  { return document.getElementById('generateImageModal'); }
function getBody()   { return document.getElementById('genImageBody'); }
function getFooter() { return document.getElementById('genImageFooter'); }
...
function chipHtml(options, selectedKey, dataAttr) {
  return options.map(o =>
    '<button type="button" class="gen-chip' + (selectedKey === o.key ? ' selected' : '') + '" ' + dataAttr + '="' + o.key + '">' +
...
function summaryTagsHtml() {
  ...
  return '<div class="gen-summary-bar">' +
...
function renderBody() {
  ...
  if (_genState === 'idle') {
    ...
    if (ta) {
      ta.addEventListener('input', () => { _promptText = ta.value; });
    }
  }
  ...
  if (_genState === 'result') {
    body.innerHTML =
      ...
      '<img class="gen-image-preview" src="' + escapeHtml(_imageUrl) + '" alt="Generated image" />' +
      summaryTagsHtml() +
